...
pub const LongListTag = struct {
    ...
    pub fn deinit(self: *LongListTag) void {
        common.internal_allocator.free(self.value);
    }

    pub fn deserialize(stream: *BinaryStream, options: ReadWriteOptions) !LongListTag {
        if (options.tag_type) {
            const read_type = try common.readTagTypeByte(stream);
            if (read_type != @intFromEnum(TagType.LongList)) return error.InvalidTagType;
        }

        const name = try common.readName(stream, options);
        const len = try common.readLength32(stream, options);

        const value = try common.internal_allocator.alloc(i64, len);
        errdefer common.internal_allocator.free(value);

        for (value) |*v| {
            v.* = try common.readInt64(stream, options);
        }

        return .{ .name = name, .value = value };
    }

    pub fn serialize(self: LongListTag, stream: *BinaryStream, options: ReadWriteOptions) !void {
        if (options.tag_type) try stream.writeInt(u8, @intFromEnum(TagType.LongList), .little);
        try common.writeName(stream, self.name, options);
        try common.writeLength32(stream, self.value.len, options);

        for (self.value) |v| {
            try common.writeInt64(stream, v, options);
        }
    }
